  ...
  return (
    <div>
      {photoUrls.length > 0 && (
        <>
          <div className="photo-row">
            {photoUrls.map((src) => (
              <img key={src} src={src} alt="" className="photo-ph" style={{ objectFit: 'cover' }} />
            ))}
          </div>
      ...
      <div className="field">
        ...
        <div>{details.rating ? `${details.rating} (${details.userRatingCount ?? 0} reviews)` : 'Not available'}</div>
      </div>

      <div className="field">
        <label>🕒 Hours</label>
        <div>{details.regularOpeningHours?.weekdayDescriptions?.join(', ') || 'Not available'}</div>
      </div>

      <div className="field">
        <label>📞 Contact</label>
        <div>{details.internationalPhoneNumber || 'Not available'}</div>
      </div>
    </div>
  );
}
